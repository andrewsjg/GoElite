...
func TestGetPlanetaryData(t *testing.T) {
	game := InitGame(false)
	pd := game.GetPlanetaryData("LAVE")

	cleanDesc := string(cleanBytes([]byte(pd.Description)))
	laveDesc := "Lave is most famous for its vast rain forests and the Laveian tree grub"

	if strings.Compare(cleanDesc, laveDesc) != 0 {
		t.Error("LAVE GoatSoup data incorrect. Expected 'Lave is most famous for its vast rain forests and the Laveian tree grub' Got:", pd.Description)
	}
