...
func initCommodities(politicallCorrect bool) []TradeGood {
	...
	commodities = append(commodities, TradeGood{0x13, -0x02, 0x06, 0x01, 0, "Food", "Fd"})
	commodities = append(commodities, TradeGood{0x14, -0x01, 0x0A, 0x03, 0, "Textiles", "Tx"})
	commodities = append(commodities, TradeGood{0x41, -0x03, 0x02, 0x07, 0, "Radioactives", "Rx"})

	if politicallCorrect {
		commodities = append(commodities, TradeGood{0x28, -0x05, 0xE2, 0x1F, 0, "Robot Slaves", "RS"})
		commodities = append(commodities, TradeGood{0x53, -0x05, 0xFB, 0x0F, 0, "Beverages", "Bx"})

	} else {
		commodities = append(commodities, TradeGood{0x28, -0x05, 0xE2, 0x1F, 0, "Slaves", "Sl"})
		commodities = append(commodities, TradeGood{0x53, -0x05, 0xFB, 0x0F, 0, "Liquor/Wines", "Wn"})

	}

	commodities = append(commodities, TradeGood{0xC4, +0x08, 0x36, 0x03, 0, "Luxuries", "Lx"})

	if politicallCorrect {
		commodities = append(commodities, TradeGood{0xEB, +0x1D, 0x08, 0x78, 0, "Rare Species", "Ra"})
	} else {
		commodities = append(commodities, TradeGood{0xEB, +0x1D, 0x08, 0x78, 0, "Narcotics", "Nx"})
	}

	commodities = append(commodities, TradeGood{0x9A, +0x0E, 0x38, 0x03, 0, "Computers", "Cx"})
	commodities = append(commodities, TradeGood{0x75, +0x06, 0x28, 0x07, 0, "Machinery", "Ma"})
	commodities = append(commodities, TradeGood{0x4E, +0x01, 0x11, 0x1F, 0, "Alloys", "Ax"})
	commodities = append(commodities, TradeGood{0x7C, +0x0d, 0x1D, 0x07, 0, "Firearms", "Fx"})
	commodities = append(commodities, TradeGood{0xB0, -0x09, 0xDC, 0x3F, 0, "Furs", "Fu"})
	commodities = append(commodities, TradeGood{0x20, -0x01, 0x35, 0x03, 0, "Minerals", "Mx"})
	commodities = append(commodities, TradeGood{0x61, -0x01, 0x42, 0x07, 1, "Gold", "Au"})
	commodities = append(commodities, TradeGood{0xAB, -0x02, 0x37, 0x1F, 1, "Platinum", "Pt"})
	commodities = append(commodities, TradeGood{0x2D, -0x01, 0xFA, 0x0F, 2, "Gem-Stones", "Gm"})
	commodities = append(commodities, TradeGood{0x35, +0x0F, 0xC0, 0x07, 0, "Alien Items", "Ai"})
